let player, currenQuestion, askeQuestions = [];
...
function win() {
  ...
  // win.style="visibility: visible; z-index: 2";
...
function gameOver() {
  if (player.nonAmount > 0) {
    alert("Вітаємо. Ви виграли " + player.nonAmount + " грн.");
  }
  else {
    alert("Ви програли");
  }
...
function answer(q) {
  ...
  playerPoints.innerText = " У вас на рахунку " + player.amount + ". Незгораєма сума " + player.nonAmount;
...
function generateQuestion() {
  ...
  while (askeQuestions.includes(num)) {
    num = Math.floor(Math.random() * questions.length);
  }
  askeQuestions.push(num);
